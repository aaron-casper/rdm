...
#    under the License.
# modified to provide 3 values and labels in .csv format
...
def speedtest():
    ...
    if not args.simple:
        pingcsv = str(('Ping: %(latency)s ms,' % best).encode('utf-8', 'ignore'))
    else:
        pingcsv = str('Ping: %(latency)s ms,' % best)
    ...
    downcsv = str('Download: %0.2f M%s/s,' %
    ...
    upcsv = str('Upload: %0.2f M%s/s,' %
           ((ulspeed / 1000 / 1000) * args.units[1], args.units[0]))
    print(pingcsv + downcsv + upcsv)
